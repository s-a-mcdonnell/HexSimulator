...
class Hex:
    ...
    def hit_neighbor(self, future, neighbors_movable, neighbors_wall, dir):
        ...
        if (neighbors_wall[dir] == 1) or (
                (neighbors_wall[(dir - 1) % 6] == 1) and (neighbors_wall[(dir + 1) % 6] == 1)):
    ...
    def update(self):
        ...
        if (hex_matrix[self.matrix_index][self.list_index].occupied == True) and (
                str(hex_matrix[self.matrix_index][self.list_index].state) == "[0, 0, 0, 0, 0, 0]"):
            future.occupied = True
        ...
        if self.movable:
            ...
            if self.list_index - 1 > 0:
                ...
                self.motion_handler(future, hex_matrix[self.matrix_index][self.list_index - 1], neighbors_movable,
                                    neighbors_wall, 0)
            ...
            if (self.matrix_index + 1 < len(hex_matrix)) and (self.list_index - 1 > 0):
                ...
                self.motion_handler(future, hex_matrix[self.matrix_index + 1][self.list_index - 1], neighbors_movable,
                                    neighbors_wall, 1)

            # NORTHWEST NEIGHBOR (5)
            if self.matrix_index - 1 > 0:
                # Call motion_handler, passing northwest neighbor
                self.motion_handler(future, hex_matrix[self.matrix_index - 1][self.list_index], neighbors_movable,
                                    neighbors_wall, 5)
...
#        elif self.state[5]:
#            self.state[5] = 0
#        elif self.state[5] == 0 & self.state[4] == 0 & self.state[3] == 0 & self.state[2] == 0 & self.state[1] == 0 & self.state[0] == 0 & self.movable:
#            self.movable = False
#        elif self.movable == False & self.occupied:
#            self.occupied = False
#        else:
#            self.make_move(0)
...
def postohex(position):
    ...
    hex_matrix[round(indx)][round(indy)].update()

    print('clicked @', round(indx), round(indy))
...
while run:
    ...
    if on:
        ...
        # first, make sure the walls are there (and unchangable (later)
        ...
        for i in range(6):
            # Top edge
            hex_matrix[1 + 2 * i][6 - i].update()
            hex_matrix[2 + 2 * i][6 - i].update()

            # Bottom edge
            hex_matrix[1 + 2 * i][15 - i].update()
            hex_matrix[2 + 2 * i][14 - i].update()
